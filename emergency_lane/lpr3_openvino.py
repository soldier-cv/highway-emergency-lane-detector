"""
HyperLPR3 车牌识别 - OpenVINO GPU 加速版
...
class LPR3DetectorOV:
    """车牌检测器 - OpenVINO GPU 加速 (YOLOv5格式)"""
    ...
    def __init__(self, model_path=None, device="GPU"):
        ...
        self.input_size = tuple(self.compiled.inputs[0].shape[2:])  # (640, 640)
    ...
    def detect(self, img, conf_threshold=0.3, nms_threshold=0.5):
        ...
        output = self.infer_request.get_output_tensor(0).data  # (1, 25200, 15)
...
class LPR3RecognizerOV:
    ...
    def __init__(self, model_path=None, device="GPU"):
        ...
        self.compiled = core.compile_model(model, device)
        self.infer_request = self.compiled.create_infer_request()
        self.input_size = tuple(self.compiled.inputs[0].shape[2:])  # (48, 160)
    ...
    def recognize(self, plate_img):
        ...
        # 推理
        self.infer_request.infer({0: data})
        
        # 后处理 - CTC解码
        output = self.infer_request.get_output_tensor(0).data  # (1, 20, 78)
...
class LPR3ClassifierOV:
    ...
    def __init__(self, model_path=None, device="CPU"):
        ...
        self.compiled = core.compile_model(model, device)
        self.infer_request = self.compiled.create_infer_request()
        self.input_size = tuple(self.compiled.inputs[0].shape[2:])
...
class LicensePlateCatcherOV:
    """HyperLPR3 替代品 - OpenVINO GPU 加速版
    ...
    def __init__(self, device="GPU", det_level=1):
        ...
        if det_level == 0:
            ...
            det_path = os.path.expanduser("~/.hyperlpr3/20230229/onnx/y5fu_640x_sim.onnx")
